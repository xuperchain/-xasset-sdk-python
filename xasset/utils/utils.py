...
class Utils(object):
    ...
    def gen_asset_id(app_id):
        return Utils.gen_id_help(app_id, 0)
    ...
    def gen_nonce():
        t = time.time()
        nano_second = int(round(t * 1000 * 1000 * 1000))
        randId1 = Utils.gen_random_id()
        randId2 = Utils.gen_random_id()
        content = '%d#%d#%d' % (randId1, randId2, nano_second)
        sign = Utils.str_sign_to_int(content)
    ...
    def gen_id_help(base_id, flag):
        ...
        s = Utils.str_sign_to_int(content)
        r1 = Utils.gen_random_id()
        r2 = Utils.gen_random_id()
        lk = base_id
